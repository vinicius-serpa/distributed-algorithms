...
public class SampleAppl {
	...
	private static Channel getRbChannel(ProcessSet processes) {
		...
		/* Create a channel. Uses default event scheduler. */
		Channel channel = myQoS
				.createUnboundChannel("Reliable Broadcast Channel");
		/*
		 * Application Session requires special arguments: filename and . A
		 * session is created and binded to the stack. Remaining ones are
		 * created by default
	...
	private static Channel getFCChannel(ProcessSet processes) {
		...
		/* Create a channel. Uses default event scheduler. */
		Channel channel = myQoS
				.createUnboundChannel("Flooding Consensus Channel");
		/*
		 * Application Session requires special arguments: filename and . A
		 * session is created and binded to the stack. Remaining ones are
		 * created by default
	...
	private static Channel getUHCChannel(ProcessSet processes) {
		...
		UniformHierarchicalConsensusSession uhcsession = (UniformHierarchicalConsensusSession) uhclayer
				.createSession();
		SampleApplSession sasession = (SampleApplSession) salayer
				.createSession();
	...
	private static Channel getCOnoWChannel(ProcessSet processes) {
		...
		Layer[] qos = { new TcpCompleteLayer(), new BasicBroadcastLayer(),
				new TcpBasedPFDLayer(),
				// new DelayLayer(),
				new LazyRBLayer(), new NoWaitingCOLayer(),
				new SampleApplLayer() };
	...
	private static Channel getUnTOChannel(ProcessSet processes) {
		...
		/* Create a channel. Uses default event scheduler. */
		Channel channel = myQoS
				.createUnboundChannel("Uniform Total Order Channel");
		/*
		 * Application Session requires special arguments: filename and . A
		 * session is created and binded to the stack. Remaining ones are
		 * created by default
	...
	/**
	 * Builds a new Appia Channel with Consensus-based Non-Blocking Atomic
	 * Commit
	 * 
	 * @param processes
	 *            set of processes
	...
	private static Channel getR1NRChannel(ProcessSet processes) {
		...
		// Shared sessions and those that require initial configuration are
		// created
		// explicitly.
