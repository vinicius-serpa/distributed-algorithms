...
package irdp.protocols.tutorialDA.lazyRB.custom;
...
public class LazyRBSession extends Session {
	...
	private void handleProcessInitEvent(ProcessInitEvent event) {
		...
		// Mantém um conjunto de processos corretos
		...
		/* Armazena mensagens já enviadas por outros processos para retransmissão em caso de falha
	...
	private void bebDeliver(SendableEvent event) {
		...
		if (!delivered.contains(msgID)) {
			...
			/* Não precisa manter a lista de processos emissores
			...
			*/
			...
			// Retransmite independente de o emissor ter falhado
			// if (!pi.isCorrect()) {
	 ...
	 */
	/*
	 * private void handleCrash(Crash crash) { int pi =
	 * crash.getCrashedProcess(); System.out.println("Process " + pi +
	 * " failed.");
	 * 
	 * try { crash.go(); } catch (AppiaEventException ex) {
	 * ex.printStackTrace(); }
	 * 
	 * // changes the state of the process to "failed"
	 * processes.getProcess(pi).setCorrect(false);
	 * 
	 * // resends the messages of the failed process for(SendableEvent event :
	 * from[pi]) bebBroadcast(event); from[pi].clear(); }
	 */
}
